		...
		switch (boundaryscheme) {
			...
			case "By Scale":
				...
				for (let bv of this.boundary.adjacentVertices()) {
					...
					//if (theta<Math.PI/4){sf = 1/Math.cos(theta);}
					//else if (theta<3 * Math.PI/4){sf = 1/Math.sin(theta);}
					//else if (theta<5 * Math.PI/4){sf = -1/Math.cos(theta);}
					//else if (theta<7 * Math.PI/4){sf = -1/Math.sin(theta);}
					//else if (theta<2 * Math.PI){sf = 1/Math.cos(theta);}
					
					sf = Math.min(Math.abs(1/Math.cos(theta)), Math.abs(1/Math.sin(theta)));
					
					// sf = Math.abs(Math.cos(theta));
